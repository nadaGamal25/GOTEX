...
export default function SmsaShippments(userData) {
  ...
  async function sendOrderDataToApi() {
    ...
    try {
      const response = await axios.post(
        "https://dashboard.go-tex.net/test/smsa/create-user-order",
        orderData,
        {
          headers: {
  ...
  async function getClientsList() {
    ...
    //   console.log(localStorage.getItem('userToken'))
    //   try {
    //     const response = await axios.get('https://dashboard.go-tex.net/test/glt/cities',
    //     {
    //       headers: {
    //         Authorization: `Bearer ${localStorage.getItem('userToken')}`,
    //       },
    ...
    //         link.href = `https://dashboard.go-tex.net/test${stickerUrl}`;
    //         link.target = "_blank";
    //         link.download = "";
...
<div className='d-flex align-items-center pb-3'>
  ...
  {item.data.waybills ?( item.data.waybills.map((sticker, index) => (
    ...
    {stickerUrls.map((sticker,index)=>{
          <li><a class="dropdown-item" href={`https://dashboard.go-tex.net/test${sticker}`}>استيكر </a></li>
