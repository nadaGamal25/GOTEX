...
const [dateFilter, setDateFilter] = useState(false);
  ...
  const [endDate, setEndDate] = useState('');
    ...
    async function getShipmentsAdmin() {
        try {
          setLoading(true);
          const response = await axios.get(`https://dashboard.go-tex.net/test/orders`, {
            params: {
                page: currentPage,
                limit: 30,
  ...
  try {
    ...
    const dataToExport = response.data.data.map((item, index) => {
      return [
        ...
        item.weight || '_',
        item.status || '_',
        item.marketer && item.marketer.length > 0 && item.marketer[0].name|| '_',
        item.cancelReason || '_',
...
//       `https://dashboard.go-tex.net/test/orders/filter-by-client-data?keyword=${clientFilter}`,
//       {
...
//     setLoading(true);
//     const response = await axios.get(
//       `https://dashboard.go-tex.net/test/orders/filter-by-marketercode?marktercode=${marketerCodeFilter}`,
//       {
...
//     setLoading(true);
//     const response = await axios.get(
//       `https://dashboard.go-tex.net/test/orders/filter-by-date?startDate=${startDate}&endDate=${endDate}`,
//       {
