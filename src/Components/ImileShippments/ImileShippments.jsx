  ...
  async function sendOrderDataToApi() {
    ...
    try {
      const response = await axios.post(
        "https://dashboard.go-tex.net/test/imile/create-user-order",
        {
            ...orderData,
            skuDetailList: theSkuDetailList
          },
...
//   }
  ...
  async function getClientsList() {
    try {
      const response = await axios.get('https://dashboard.go-tex.net/test/daftra/get-markter-clints',
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('userToken')}`,
        },
  ...
  function openBase64PDFInNewWindow(base64String) {
    ...
    // window.open(pdfUrl);
    const stickerUrl = `https://dashboard.go-tex.net/test/${pdfUrl}`;
