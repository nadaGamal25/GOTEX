...
const [marketerid, setMarketerid] = useState('');
      ...
      async function getUsersListsAdmin() {
        try {
          const response = await axios.get('https://dashboard.go-tex.net/test/admin/get-all-users',
          {
            headers: {
      ...
      async function addDepositToUser() {
        try {
          const response = await axios.post(
            'https://dashboard.go-tex.net/test/admin/add-deposit-to-user',
            {
              id: selectedUserId,
      ...
      async function proofCR() {
        try {
          const response = await axios.post(
            'https://dashboard.go-tex.net/test/admin/proof-user-cr',
            {
              email: emailCR,
            },
            {
      ...
      async function proofCR3() {
        try {
          const response = await axios.post(
            'https://dashboard.go-tex.net/test/admin/un-proof-user-cr',
            {
              email: emailCR3,
            },
            {
      ...
      async function connectMarketerWithDaftra() {
        try {
          const response = await axios.post(
            'https://dashboard.go-tex.net/test/daftra/connect-markter-with-daftra',
            {
              daftraid: daftraid,
          ...
          return search === ''? item : item.email.includes(search);
            ...
            return(
              <tr key={index}>
                ...
                {item.cr && item.cr[0] ? (
        <td><a className='text-danger' href={item.cr[0].replace('public', 'https://dashboard.go-tex.net/test')} target='_blank'>ملف التوثيق</a>
          </td>
      ) : (
        <td>_</td>
