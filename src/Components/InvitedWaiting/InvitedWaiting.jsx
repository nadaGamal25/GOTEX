...
export default function InvitedWaiting() {
  ...
  useEffect(() => {
    getWaitingListsAdmin();
        ...
        try {
          const response = await axios.get(`https://dashboard.go-tex.net/test/invatation/accept-invitation/${orderId}`,
          {
            headers: {
  ...
  async function getWaitingListsAdmin() {
    try {
      const response = await axios.get(
        'https://dashboard.go-tex.net/test/invatation/get-invitations-wait',
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('userToken')}`,
          },
